...
// Define an interface for Pinecone index
...
// Initialize the embedding service and Pinecone service
...
async function seedDatabase() {
  try {
    ...
    // await pineconeService.deleteProductVector(0); // Adjust according to logic for clearing Pinecone vectors
    // console.log("Cleared all vectors in the Pinecone namespace.");

    const filePath = "Data-18-Feb/Product_rows.csv"; // Update this path
    const records: any[] = [];
    ...
    for (const record of records) {
      ...
      await pineconeService.upsertProductVector(id, embedding, { name, description, categories, price });
